...
def _validate_workflow(workflow: dict[str, Any] | None) -> dict[str, Any]:
    """Validate an optional additive contract/selection/QA manifest extension."""
    if workflow is None:
        return {}
    if not isinstance(workflow, dict):
        raise CliError("workflow must be a dictionary")
    allowed = {"figure_contract", "chart_selection", "qa"}
    unknown = set(workflow).difference(allowed)
    if unknown:
        raise CliError(
            "workflow contains unsupported fields: " + ", ".join(sorted(unknown))
        )
    try:
        encoded = json.dumps(workflow, sort_keys=True).encode("utf-8")
    except (TypeError, ValueError) as exc:
        raise CliError(f"workflow must be JSON serializable: {exc}") from exc
    if len(encoded) > 1_000_000:
        raise CliError("workflow metadata is limited to 1,000,000 bytes")
    return workflow
...
def export_figure(
    ...
    *,
    ...
    workflow_document = _validate_workflow(workflow)
