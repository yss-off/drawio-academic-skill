...
```text
学术需求 -> preflight + 交付类别 -> 复杂任务可比较布局候选 -> YAML spec -> sibling base CLI 校验 -> 对应类别产物，sidecars/manifest 保存在 .drawio-tmp/<name>/
```

所有类别都交付可编辑 `.drawio`。主交付类别三选一：`raster-publication` → 300ppi 有效分辨率 PNG；`vector-submission` → 满足 venue 规则的 PDF 或文字转路径 SVG；`draft-preview` → 明确标注为预览的 live-text SVG。额外格式只作 companion。
...
- 主交付类别；出版目标未确定时使用 `draft-preview`
...
没有 Desktop 时，必须交付 PNG 就走 browser-rasterized derivative gate；没有合规矢量导出器时，把 `vector-submission` 标为 blocked，并从 `.drawio` 生成 diagrams.net 编辑 URL：
