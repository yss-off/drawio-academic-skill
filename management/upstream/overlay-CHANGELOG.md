...
### Scenario-based academic delivery contract

- Replaced competing universal PNG/SVG defaults with one authoritative delivery
  matrix: `raster-publication`, `vector-submission`, or `draft-preview`; editable
  `.drawio` remains mandatory for every class.
- Added manifest-level delivery-class validation and artifact gates for PNG,
  PDF/path-only SVG, and preview SVG.
- Updated entrypoints, playbook, export checklist, READMEs, agents, and evals to
  use the same contract. Historical release notes below remain unchanged.

### Layout planning and evidence manifests

- Added deterministic 2–3 option layout planning for complex architecture,
  workflow, and roadmap figures without changing scientific content.
- Added a small offline, license-tracked index of bundled examples/templates.
- Added a machine-readable figure manifest, artifact hashing, strict evidence
  validation, known-bad fixtures, and offline smoke tests.
